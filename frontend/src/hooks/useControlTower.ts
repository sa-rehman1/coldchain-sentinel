...
export const useDemoRun = (runId: string | null, identity: LocalIdentity) => useQuery({ queryKey: ['demo-run', runId], queryFn: ({ signal }) => controlTowerDataSource.getDemoRun(runId!, identity, signal), enabled: Boolean(runId), refetchInterval: (query) => query.state.data?.status === 'ACCEPTED' || query.state.data?.status === 'PROCESSING' ? 750 : false });
