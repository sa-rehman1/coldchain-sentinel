...
const columns = columnHelper.columns([
  columnHelper.display({ id: 'select', header: '', enableSorting: false, cell: ({ row }) => <input type="checkbox" aria-label={`Select ${row.original.id}`} onClick={(event) => event.stopPropagation()} /> }),
  ...
  columnHelper.accessor('temperature', { header: 'Temperature', cell: ({ row }) => <span className="queue-temperature"><strong className={row.original.severity === 'critical' ? 'text-critical' : ''}>{row.original.temperature.toFixed(1)}°C</strong><small>{row.original.allowedMin}–{row.original.allowedMax}°C allowed</small></span> }),
  columnHelper.accessor('breachMinutes', { header: 'Breach', cell: ({ getValue }) => `${String(getValue())} min` }),
...
export function IncidentQueue() {
  ...
  return <div className={`page queue-page density-${density}`}>
    ...
    {selected && <div className="drawer-backdrop" onMouseDown={() => setSelected(null)}><aside className="preview-drawer" aria-label="Investigation preview" onMouseDown={(event) => event.stopPropagation()}><header><div><SeverityIndicator severity={selected.severity} /><h2>{selected.id}</h2><p>{selected.title}</p></div><button className="icon-button" onClick={() => setSelected(null)} aria-label="Close preview"><X size={18} /></button></header><div className="drawer-content"><div className="drawer-temp"><strong>{selected.temperature.toFixed(1)}°C</strong><span>Allowed {selected.allowedMin}–{selected.allowedMax}°C</span></div><dl><div><dt>Shipment</dt><dd>{selected.shipment}</dd></div><div><dt>Product</dt><dd>{selected.product}</dd></div><div><dt>Sensor</dt><dd>{selected.sensor}</dd></div><div><dt>Breach duration</dt><dd>{selected.breachMinutes} minutes</dd></div><div><dt>Governance</dt><dd><GovernanceIndicator decision={selected.governance} /></dd></div><div><dt>Reviewer</dt><dd>{selected.reviewer ?? 'Unassigned'}</dd></div></dl><div className="drawer-notice"><strong>Preview only</strong><span>Consequential decisions are available only in the full investigation workspace.</span></div></div><footer><Button variant="primary" onClick={() => navigate(`/investigation/${selected.id}`)}>Open investigation <ArrowRight size={15} /></Button><Button variant="ghost" onClick={() => setSelected(null)}>Close</Button></footer></aside></div>}
  </div>;
}
